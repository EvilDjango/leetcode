 ...
 * @date 2020-03-08
 */
public class Topic052 {
    public static class Solution1 {
        public int totalNQueens(int n) {
            int[] total = new int[1];
            backTrack(n, 0, new boolean[n], new boolean[2 * n - 1], new boolean[2 * n - 1], total);
            return total[0];
        }

        private void backTrack(int n, int i, boolean[] col, boolean[] leftDiagonal, boolean[] rightDiagonal, int[] total) {
            if (i == n) {
                total[0]++;
                return;
            }
            ...
            for (int j = 0; j < n; j++) {
                ...
                backTrack(n, i + 1, col, leftDiagonal, rightDiagonal, total);
                col[j] = false;
